...
async def run_download(task: DLTask):
    async with sem:
        ...
        try:
            ...
            if pre_info:
                entry = (pre_info.get("entries") or [pre_info])[0]
                task.title       = pre_info.get("title")    or entry.get("title", "Unknown")
                task.channel     = pre_info.get("uploader") or entry.get("uploader", "Unknown")
                ...
                if "entries" in pre_info:
                    task.episode = f"プレイリスト {len(pre_info['entries'])}本"
                await bcast({"event": "progress", "task": task.to_dict()})
            ...
            def _run():
                with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                    ...
                    if raw_info:
                        ...
                        info_holder["entry"] = (raw_info.get("entries") or [raw_info])[0]
